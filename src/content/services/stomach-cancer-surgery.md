---
title: "Gastric Cancer Surgery (Gastrectomy)"
h1: "Gastric Cancer Surgery (Gastrectomy)"
h1Html: "Gastric cancer surgery, <em>curative</em> intent"
slug: "stomach-cancer-surgery"
category: "oncology"
shortDescription: "Surgical treatment of stomach cancer by Dr. Dwarakanath Reddy at Apollo Nellore. Subtotal and total gastrectomy with D2 lymph node dissection."
procedureType: "Gastrectomy with D2 Lymphadenectomy"
typicalDuration: "3 to 5 hours"
hospitalStay: "7 to 10 days"
recoveryTime: "Return to work in 2 to 3 weeks, full recovery over 2 to 3 months"
anesthesia: "General anaesthesia"
approach: "mixed"
bodyLocation: "Stomach"
medicalSpecialty:
  - "Surgical Oncology"
  - "Surgical Gastroenterology"
phase: "1"
order: 40
heroIntro: "Surgery is the cornerstone of curative treatment for stomach cancer. The right operation, performed at the right stage, gives the best chance of long term survival. Dr. Reddy performs both open and laparoscopic gastrectomy at Apollo Nellore."
heroCta:
  label: "Send your reports on WhatsApp"
  message: "Hi Dr. Reddy, I have a stomach biopsy or endoscopy report I'd like reviewed."
sections:
  - type: list
    title: "When is surgery used?"
    intro: "Surgery is the primary treatment for stomach cancers that are:"
    items:
      - "Limited to the stomach or nearby lymph nodes"
      - "Not yet spread to distant organs"
      - "In patients fit enough for surgery"
    outro: "For more advanced cases, neoadjuvant chemotherapy (before surgery) is often used to shrink the tumor and improve outcomes."
  - type: list
    title: "Types of gastrectomy"
    items:
      - "Subtotal gastrectomy: removes part of the stomach (for tumors in the lower stomach)"
      - "Total gastrectomy: removes the entire stomach (for tumors in the upper stomach or large tumors)"
      - "D2 lymphadenectomy: thorough removal of lymph nodes around the stomach (gold standard for cure)"
    outro: "Laparoscopic gastrectomy is offered for selected patients. Open surgery remains the standard for very advanced cases."
  - type: timeline
    title: "What to expect"
    entries:
      - period: "Hospital stay"
        detail: "7 to 10 days."
      - period: "Diet"
        detail: "Liquid then soft diet for 4 to 6 weeks."
      - period: "Long term"
        detail: "Smaller, more frequent meals."
      - period: "Supplementation"
        detail: "Vitamin B12 if total gastrectomy was performed."
      - period: "Oncology follow up"
        detail: "Every 3 to 6 months."
  - type: list
    title: "Why Dr. Reddy for stomach cancer surgery"
    items:
      - "Surgical Gastroenterology specialisation includes GI oncology"
      - "D2 lymph node dissection performed as standard"
      - "Apollo Nellore tumor board reviews every case"
      - "Coordinated chemotherapy and radiation if needed"
      - "Honest staging assessment before deciding surgical approach"
  - type: faqs
    title: "Frequently asked questions"
    items:
      - question: "Can I live without a stomach?"
        answer: "Yes. After total gastrectomy, the small intestine adapts. You will eat smaller meals more frequently but can lead a normal life."
      - question: "Will I need chemotherapy?"
        answer: "Most patients with gastric cancer benefit from chemotherapy before, after, or both around surgery. The Apollo oncology team will design a personalised plan."
      - question: "Is laparoscopic gastrectomy as effective as open?"
        answer: "For early and selected intermediate cases, yes. For advanced cases, open surgery often remains the safer choice."
closingCta:
  prompt: "Have a recent endoscopy, biopsy, or CT report? Send it on WhatsApp for a clear next step."
  label: "Send your reports on WhatsApp"
  message: "Hi Dr. Reddy, I have endoscopy, biopsy, or CT reports for stomach cancer. Please review."
seo:
  title: "Stomach Cancer Surgery in Nellore"
  description: "Surgical treatment of stomach cancer by Dr. Dwarakanath Reddy at Apollo Nellore. Subtotal and total gastrectomy with D2 lymph node dissection. Multidisciplinary cancer care."
  keywords:
    - "stomach cancer surgery Nellore"
    - "gastrectomy Apollo Nellore"
    - "gastric cancer surgeon Andhra Pradesh"
    - "D2 lymphadenectomy"
---

Gastrectomy for stomach cancer at Apollo Speciality Hospitals, Nellore, by Dr. Dwarakanath Reddy Vembuluru.
