---
title: "Bariatric (Weight Loss) Surgery in Nellore"
h1Secondary: "Gastric bypass and sleeve gastrectomy · Apollo Nellore"
h1: "Bariatric (Weight Loss) Surgery in Nellore"
h1Html: "Bariatric surgery, <em>metabolic</em> first"
slug: "bariatric-surgery"
shortDescription: "Laparoscopic Gastric Bypass and Sleeve Gastrectomy by Dr. Dwarakanath Reddy at Apollo Nellore. Long term weight loss and metabolic recovery."
heroIntro: "Bariatric surgery is the most effective long term treatment for severe obesity and obesity related diseases like Type 2 diabetes. Dr. Reddy offers Gastric Bypass and Sleeve Gastrectomy at Apollo Nellore, supported by a full multidisciplinary bariatric care programme."
whatWeCover:
  title: "What we offer"
  items:
    - title: "Laparoscopic Sleeve Gastrectomy"
      href: "/gi-services/bariatric-surgery/sleeve-gastrectomy"
      description: "Restrictive procedure. Shorter operation, simpler recovery."
    - title: "Laparoscopic Roux-en-Y Gastric Bypass"
      href: "/gi-services/bariatric-surgery/gastric-bypass"
      description: "Restriction plus bypass. Strong diabetes remission outcomes."
    - title: "Revision Bariatric Surgery"
      description: "Planned for Phase 2. Conversion or revision of a previous bariatric procedure."
sections:
  - type: list
    title: "Who qualifies?"
    items:
      - "BMI 35 or higher with comorbidities (Type 2 diabetes, hypertension, sleep apnea, PCOS)"
      - "BMI 40 or higher with or without comorbidities"
      - "Persistent obesity despite serious lifestyle and medical efforts"
  - type: list
    title: "Why Dr. Reddy for bariatric surgery"
    items:
      - "Trained bariatric surgeon"
      - "Apollo Nellore offers full multidisciplinary care: nutritionist, psychologist, endocrinologist"
      - "Honest patient selection (will guide you to the right procedure or away from surgery if needed)"
      - "Lifelong follow up programme"
closingCta:
  prompt: "Curious if you qualify for bariatric surgery? Share your height, weight, and health conditions on WhatsApp."
  label: "Check your eligibility on WhatsApp"
  message: "Hi Dr. Reddy, I'd like to know if I qualify for bariatric surgery. My height is __, weight __, and health conditions include __."
order: 60
seo:
  title: "Bariatric Surgery in Nellore | Dr. Dwarakanath Reddy V"
  description: "Bariatric surgery in Nellore by Dr. Dwarakanath Reddy at Apollo Speciality Hospitals. Laparoscopic Gastric Bypass and Sleeve Gastrectomy with full multidisciplinary support."
  keywords:
    - "bariatric surgery Nellore"
    - "weight loss surgery Apollo Nellore"
    - "gastric bypass Nellore"
    - "sleeve gastrectomy Andhra Pradesh"
---

Bariatric surgery at Apollo Speciality Hospitals, Nellore, by Dr. Dwarakanath Reddy Vembuluru.
